    ...
    **ap_args
    ...
    if not configfile:
        configfile = os.path.join(
            os.path.dirname(__file__), 'testdata', 'config.yaml'
        )
    if not configdir:
        configdir = os.path.join(
            os.path.dirname(__file__), 'testdata', 'authprogs.d'
        )
...
class AuthProgsTests(unittest.TestCase):
    ...
    def test_client_ip_cidr(self):
        """Test that a CIDR network range matches a sample address."""
        ap = getap('10.11.12.13', 'CIDR')
        self.assertEqual(ap.find_match(), {'command': ['CIDR']})

    def test_client_ip_cidr_neg(self):
        """Test that a CIDR network range doesn't match a sample address."""
        ap = getap('0.1.2.3', 'CIDR')
        self.assertRaises(authprogs.CommandRejected, ap.find_match)
    ...
    def test_keyname_nospace(self):
        ...
        self.assertRaises(
            authprogs.Error, authprogs.AuthProgs, keyname='foo bar'
        )
    ...
    def test_unrestricted_client_ip(self):
        ...
        self.assertEqual(ap.find_match(), {'command': ['NO_IP_RESTRICTIONS']})
    ...
    def test_command_with_args(self):
        ...
        self.assertEqual(
            ap.find_match(), {'command': ['/bin/echo', 'Hello', 'World']}
        )
    ...
    def test_pcre_command(self):
        ...
        for command in [
            ...
            self.assertEqual(ap.find_match(), {'command': command.split()})
    ...
    def test_unrestricted_scp(self):
        ...
        ap = getap('0.0.0.6', 'scp -d -t /etc/passwd')
        self.assertEqual(
            ap.find_match(), {'command': ['/usr/bin/scp', '-d', '-t', '/etc/passwd']}
        )
    ...
    def test_recursive_scp(self):
        ...
        self.assertEqual(
            ap.find_match(), {'command': ['/usr/bin/scp', '-r', '-t', '--', '/tmp']}
        )
    ...
    def test_permissions_scp(self):
        ...
        ap = getap('0.0.0.7', 'scp -p -r -t /tmp')
        self.assertEqual(
            ap.find_match(), {'command': ['/usr/bin/scp', '-p', '-r', '-t', '/tmp']}
        )
    ...
    def test_install_key_with_logfile(self):
        ...
        expected = (
            ...
            'no-port-forwarding {}\n'.format(initial_contents, keydata)
        )
    ...
    def test_install_key_duplicate(self):
        ...
        thefile = io.StringIO()
        initial_contents = (
            'command="" ssh-rsa AAAABBBBCCCCDD ' 'foo@example.com\n'
        )
        thefile.write(initial_contents)
    ...
    def test_files_scp(self):
        ...
        self.assertEqual(
            ap.find_match(), {'command': ['/usr/bin/scp', '-f', '--', '/etc/aliases']}
        )
        ...
        self.assertEqual(
            ap.find_match(), {'command': ['/usr/bin/scp', '-f', '--', '/etc/group']}
        )
        ap = getap('1.0.0.10', 'scp -f -- /etc/resolv.conf')
        self.assertEqual(
            ap.find_match(),
            {'command': ['/usr/bin/scp', '-f', '--', '/etc/resolv.conf']},
        )
        ...
        ap = getap('1.0.0.10', 'scp -f /etc/resolv.conf')
        self.assertEqual(
            ap.find_match(), {'command': ['/usr/bin/scp', '-f', '/etc/resolv.conf']}
        )
        ap = getap('1.0.0.10', 'scp -f /etc/foo/')
        self.assertRaises(authprogs.CommandRejected, ap.find_match)
    ...
    def test_load(self):
        ...
        self.assertEqual(ap.find_match(), {'command': ['/bin/echo', 'Goodbye']})

        # From one file in config directory
        ap = getap('0.0.0.0', '100-config', logfile='/tmp/b')
        self.assertEqual(ap.find_match(), {'command': ['100-config']})

        # From another file in config directory
        ap = getap('0.0.0.0', '200-config')
        self.assertEqual(ap.find_match(), {'command': ['200-config']})
